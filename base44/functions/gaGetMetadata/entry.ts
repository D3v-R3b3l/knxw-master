...
const ANALYTICS_DATA_API = 'https://analyticsdata.googleapis.com';
...
Deno.serve(async (req) => {
  try {
    ...
    if (!property_id) return new Response(JSON.stringify({ error: 'property_id is required' }), { status: 400 });

    const accessToken = await getValidAccessToken(base44, user.id);

    const response = await fetch(`${ANALYTICS_DATA_API}/v1beta/properties/${property_id}/metadata`, {
      headers: { 'Authorization': `Bearer ${accessToken}` },
    });

    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(`Google API error: ${errorData.error?.message || 'Failed to fetch metadata'}`);
    }

    const metadata = await response.json();

    const dimensions = (metadata.dimensions || []).map(d => ({ apiName: d.apiName, uiName: d.uiName, description: d.description, category: d.category }));
    const metrics = (metadata.metrics || []).map(m => ({ apiName: m.apiName, uiName: m.uiName, description: m.description, category: m.category, type: m.type }));

    return new Response(JSON.stringify({ dimensions, metrics }), {
      headers: { 'Content-Type': 'application/json' },
    });

  } catch (error) {
    console.error('[gaGetMetadata] Error:', error);
