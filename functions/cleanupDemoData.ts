...
Deno.serve(async (req) => {
  try {
    ...
    for (const entityName of demoEntities) {
      try {
        // Try to filter by is_demo: true
        let demoRecords = [];
        try {
          demoRecords = await service.entities[entityName].filter({ is_demo: true }, null, 500);
        } catch (filterError) {
          // If is_demo field doesn't exist on this entity, skip gracefully
        ...
        if (!dry_run && confirm && demoRecords.length > 0) {
          ...
          for (const record of demoRecords) {
            try {
              ...
              console.error(`Failed to delete ${entityName} ${record.id}:`, deleteError.message);
