...
Deno.serve(async (req) => {
  try {
    ...
    const { property_id, dimensions = ['pagePath'], metrics = ['screenPageViews'], start_date = '7daysAgo', end_date = 'today' } = body;
    ...
    const url = `${ANALYTICS_DATA_API}/v1beta/properties/${encodeURIComponent(property_id)}:runReport`;
    ...
    if (!res.ok) {
      ...
      return Response.json({ error: `GA4 Data API error: ${errorText}` }, { status: res.status });
    }
    
    const data = await res.json();
    return Response.json({ report: data });
  } catch (error) {
    console.error('[gaRunReport] Error:', error);
