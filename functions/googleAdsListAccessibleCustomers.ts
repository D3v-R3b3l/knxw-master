...
Deno.serve(async (req) => {
  try {
    ...
    const res = await fetch('https://googleads.googleapis.com/v14/customers:listAccessibleCustomers', {
    ...
    const customerData = (data.resourceNames || []).map(rn => rn.replace('customers/', ''));

    return Response.json({ customers: customerData });

  } catch (error) {
    console.error('[googleAdsListAccessibleCustomers] Error:', error);
